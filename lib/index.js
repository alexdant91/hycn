class Hycn {
    constructor(application_id, secret_key) {
        ...
        this.link = {
            shorten: (token, long_url, options, done) => {
                return this.getShortLink(token, long_url, options, done)
            },
            direct: (token, long_url, options, done) => {
                return this.getShortLinkDirect(token, long_url, options, done)
            },
    ...
    getShortLink(token, long_url, options, done) {
        ...
        options = options != '' && options != null && options != undefined && typeof options === 'object' ? options : null;
        let body = {};
        if (options != null) body.sub_user_id = options.sub_user_id;
        if (long_url != false) body.long_url = long_url;
        if (token && long_url) {
            this.rp({
                method: 'POST',
                headers: {
    ...
    getShortLinkDirect(token, long_url, options, done) {
        token = token != '' && token != null && token != undefined && typeof token === "string" ? token : false;
        long_url = long_url != '' && long_url != null && long_url != undefined && typeof long_url === "string" ? long_url : false;
        options = options != '' && options != null && options != undefined && typeof options === 'object' ? options : null;
        let body = {};
    ...
    paginateAllShortLink(token, param, done) {
        ...
        if (token) {
            ...
            const sub_user_id = param.sub_user_id != '' && param.sub_user_id != null && param.sub_user_id != undefined ? param.sub_user_id : null;
            const uri = sub_user_id != null ? `http://api.${this.host}/v1/link/paginate?page=${page}&limit=${limit}&sub_user_id=${sub_user_id}` : `http://api.${this.host}/v1/link/paginate?page=${page}&limit=${limit}`;
            this.rp({
                method: 'GET',
                headers: {
                    "Token": token,
